import { supabase } from "@/integrations/supabase/client";

export interface TierInfo {
  name: string;
  color: string;
  discount: number;
  minPoints: number;
  icon: string;
}

export async function getUserTier(lifetimePoints: number): Promise<TierInfo> {
  // Fetch tier settings from database
  const { data: settings } = await supabase
    .from("reward_settings")
    .select("*")
    .in("setting_key", [
      "tier_bronze_min", "tier_bronze_discount",
      "tier_silver_min", "tier_silver_discount",
      "tier_gold_min", "tier_gold_discount",
      "tier_platinum_min", "tier_platinum_discount"
    ]);

  if (!settings) {
    // Default tier if settings not found
    return {
      name: "Bronze",
      color: "text-orange-600",
      discount: 5,
      minPoints: 0,
      icon: "award"
    };
  }

  const settingsMap: any = {};
  settings.forEach(setting => {
    settingsMap[setting.setting_key] = parseInt(setting.setting_value);
  });

  const tiers = [
    {
      name: "Platinum",
      color: "text-purple-500",
      discount: settingsMap.tier_platinum_discount || 20,
      minPoints: settingsMap.tier_platinum_min || 2000,
      icon: "crown"
    },
    {
      name: "Gold",
      color: "text-yellow-500",
      discount: settingsMap.tier_gold_discount || 15,
      minPoints: settingsMap.tier_gold_min || 1000,
      icon: "star"
    },
    {
      name: "Silver",
      color: "text-gray-400",
      discount: settingsMap.tier_silver_discount || 10,
      minPoints: settingsMap.tier_silver_min || 500,
      icon: "trophy"
    },
    {
      name: "Bronze",
      color: "text-orange-600",
      discount: settingsMap.tier_bronze_discount || 5,
      minPoints: settingsMap.tier_bronze_min || 0,
      icon: "award"
    }
  ];

  // Find the highest tier the user qualifies for
  for (const tier of tiers) {
    if (lifetimePoints >= tier.minPoints) {
      return tier;
    }
  }

  // Default to Bronze
  return tiers[tiers.length - 1];
}

export function calculateDiscountedPoints(originalPoints: number, discount: number): number {
  return Math.round(originalPoints * (1 - discount / 100));
}